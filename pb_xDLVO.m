...
function f=pb_xDLVO(I,a, T,qsi, phip, Ii, rho_lig, Mw,A)
...
zp=zp*(1+1/(kappa*a));
while dh<=numel(h)
    Vvdw(dh)=A*a*a/(6*h(dh)*(a+a)); 
   %Vvdw(dh)=A*a/(12*h(dh)*(1+14*h(dh)/lambda));
   ...
   Vedl(dh)=64*pi*epsilon0*epsilon*Kb^2*T^2*(a*a/(a+a))/(Z^2*e^2)*gamma^2*exp(-Cinv*h(dh));
